...
The plugin supports importing CouchDB and encrypted-vault settings from an upstream LiveSync setup URI generated by `utils/flyio/generate_setupuri.ts` or copied from a current upstream setup export flow.

Current scope:

- setup URI decoding supports the upstream fixed-salt V2 AES-256-GCM payloads and the newer HKDF-salted AES-256-GCM payloads
